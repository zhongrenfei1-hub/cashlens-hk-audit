...
> 香港 SME 银行月结单 AI 审计分析工具 · 单文件 · 纯前端 · 支持 9 家 AI 服务商
...
| **Anthropic Claude** | ❌ 需 VPN | 原生 PDF | 🌟 唯一原生支持 PDF,审计准确度最高 |
...
把 PDF 月结单拖到网页里 → AI 按 HKSA 230 / SME-FRS 规则分类 → 输出 6 大模块报告 + 4 张 Excel Sheet 的 TSV 数据 → 复制到 `统计流水模版.xlsx` 即可。
...
- ✅ **追问**:第一次分析后,可继续对话("列出大额交易"、"导出 BIR51")
...
### Q: 想改审计规则
设置 → 高级:系统提示词 v4.0 → 直接编辑,保存后下次分析自动应用。改完不满意可点"还原默认"。
...
私有,仅供 Cashlens 部署使用。提示词内容遵守 HKSA / IRD 公开规则。

## 🔗 相关

- v4.0 提示词原文:`../01_提示词/香港审计流水分析提示词_v4.0.docx`
- Excel 模版:`../02_模版/统计流水模版.xlsx`
- 命令行版本:`../03_脚本/run_audit.py`(同样的 v4.0 提示词)
