...
**最后更新:** 2026-05-11(prompt debit-only 修正:用户反馈"统计流水只算 debit 存入,不可能有负数")
**最新已推送:** debit-only 修正(prompt + 前端文案 9 处) — 见最新 commit · `git log -3`
...
| Commit | 内容 |
|---|---|
| (本次) | **prompt debit-only 修正**:用户反馈"统计流水只算 debit 存入,不可能有负数"。改 prompt 加第一原则 + 硬约束 + Sheet 1 标题 + `original_amount` 约定 + `classification` 枚举去"出账" + Sheet 3 分组去"出账" + 初始消息文案 + 前端 2 处文案。AI 现在 credit 行整行跳过 |
| 上次 | Sheet 5 · Miheng_Movement_Summary 月度外币运动汇总:prompt 加 schema(META 头 + 9 列 + row_type 枚举 + 加总自验)+ 前端 `buildTsvSheet` 加 META 解析 / row_type 列隐藏 / `fc_*` 数字格式 |
