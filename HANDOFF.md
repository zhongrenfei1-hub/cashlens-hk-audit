...
**最后更新:** 2026-05-11(**v8.2 做账审计提醒事项**:prompt 加 [做账审计提醒规则] 段,识别 5 类做账审计复杂判断嫌疑项(资本利得/境外/股息/知识产权/政府补贴),advisory 模式 — 列出嫌疑项 + 建议补充凭证,但绝不下税务结论)
**最新已推送:** v8.2 做账审计提醒 — 见最新 commit · `git log -5`
...
9. **【做账审计提醒事项】清单**(若有 — 5 类嫌疑项 advisory:资本利得 / 境外收入 / 股息 / 知识产权 / 政府补贴 · v8.2 新增,详见 4.14)
10. **末尾固定提示语**(必须照搬,两句):
...
### 4.14 做账审计提醒事项 · v8.2 新增(统计流水板块 advisory)

**目的**:用户反馈"资本利得 vs 经营收入 / 应税 vs 境外 / 股息境内外 / IRO/FSIE 分类 / 知识产权"这些**银行流水信息不足以判断**的项,需要在统计流水阶段就**单独标记 + 提醒客户**,而不是装看不见。

**机制(advisory ≠ conclusion)**:
- AI 在统计流水分类完成后,**额外**识别 5 类嫌疑项
- 仅给"嫌疑标签 + 建议补充凭证",**绝不**下"应税/豁免"结论或引法条编号(`v8.0 硬约束 2 加例外允许 advisory`)
- 输出在【待用户确认事项】之后、末尾固定提示语之前
- 5 类都 0 笔 → 整个区块跳过

**5 类嫌疑识别特征**(详见 prompt [做账审计提醒规则] 段):
| 类别 | emoji | 识别线索 | 建议补充 |
|---|---|---|---|
| 资本利得嫌疑 | 💰 | "SALE OF / PROCEEDS / 处置 / DISPOSAL";单笔大额一次性(>1/3 月度均值) | 资产购买凭证 + 持有期 + 销售目的 |
| 境外收入嫌疑 | 🌏 | counterparty 境外 / 对手银行境外 / "OVERSEAS / OFFSHORE" | 合同地点 / 服务交付地 / 客户位置 |
| 股息 / 投资收益 | 📈 | "DIVIDEND / 股息 / 分红 / DIVD" | 持股结构 + 被投资公司注册地 |
| 知识产权 / 特许 | © | "LICENSE / ROYALTY / 授权 / 使用费" | IP 注册地 + 授权范围 |
| 政府补贴 / 资助 | 🏛️ | counterparty 含 "政府 / GOV / SUBSIDY / GRANT" | 补贴文件类型 |
...
Cashlens 仅识别嫌疑类别,无法给出精确税务结论。
**建议在「📚 做账审计板块」补充凭证后人工确认**。

💰 类别 A · 资本利得嫌疑(2 笔 · HKD 856,000)
- 涉及交易:
  · 2025-08-12 | ALPHA HOLDINGS | HKD 480,000 | 单笔大额一次性,可能处置投资
  · 2025-11-03 | BETA LTD | HKD 376,000 | 描述含 "PROCEEDS OF SALE"
- Cashlens 已分类为:其他(不代表税务口径)
- 建议补充:资产购买凭证 + 持有期 + 销售目的说明

[其他类别...]
```

**前端不需要专门 parser**:`【做账审计提醒事项】` 区块作为 markdown 显示在「📄 报告」tab(marked.js 自动渲染)。后续如要做 chip 视觉强调,加专门 regex 抓即可。

**Handoff 到做账审计板块**:`window.__latestReport` 已包含此区块,通过现有 `sendToAuditModule()` 流入 `audit.html`(全量原文)。后续 audit.html 可以专门 parse 这个区块,列出嫌疑项供客户逐笔补充凭证。
...
| Commit | 内容 |
|---|---|
| (本次 · v8.2) | **做账审计提醒事项 advisory · 5 类嫌疑识别**:prompt 加 [做账审计提醒规则] 段,识别 5 类做账审计复杂判断嫌疑项(💰 资本利得 / 🌏 境外收入 / 📈 股息 / © 知识产权 / 🏛️ 政府补贴);工作流加 step 7「做账审计提醒事项」清单(advisory 模式 · 列出嫌疑 + 建议补充凭证 + 不下税务结论)+ 末尾固定提示语更新加做账审计板块引导;硬约束 2 加例外说明(允许 advisory 但仍不引法条编号);HANDOFF section 4.14 新增 5 类识别特征 + 输出格式样例 |
| `e1fb82a` | 复制按钮 3 级 fallback — 防御沉浸式翻译等浏览器插件 / 失焦 / 旧版拦截 clipboard(L1 Clipboard API → L2 execCommand → L3 prompt 手动复制);index.html `copyText()` 重写为 async 带 fallback;audit.html 加 `safeCopyText()` |
| `a7f71d4` | handoff 鲁棒化:fallback regex 抓任意 ``` 块含 \t 当 TSV + 允许无 TSV 时降级发送 + audit.html `<pre>` 显示 rawReport |
| `2ae3632` | **v8.1 双板块架构 · 新增 audit.html 做账审计板块 + handoff 机制**:同仓库新增 `audit.html`(~300 行,独立 URL `/audit.html`);同源 localStorage 自动共享 settings / 历史项目;新增 `sendToAuditModule()` + artifact toolbar 「📤 发送到做账审计」按钮 + storage 事件实时同步;index.html 顶部 nav 加 「📚 做账审计 →」切换链接;HANDOFF section 4.13 双板块架构说明 |
